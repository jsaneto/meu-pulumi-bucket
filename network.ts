...
export const createSecurityGroup = (vpcId: pulumi.Input<string>, nameSuffix: string) => {
    // Mudamos de `sg-${nameSuffix}` para algo como `sec-group-${nameSuffix}`
    return new aws.ec2.SecurityGroup(`sec-group-${nameSuffix}`, {
        vpcId: vpcId,
        // O campo 'name' também não pode começar com sg-
        name: `security-group-${nameSuffix}`, 
